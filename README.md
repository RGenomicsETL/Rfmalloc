...
lowering that agrees with upstream PyTorch on all four released MANE
checkpoints and, on equal pinned cores, runs 1.6 to 1.9 times upstream’s
CPU throughput on the 400 nt and 2000 nt models. It stays unexported
because its R API is a separate design question. The ESM-2 native
lowering remains explicit work. Evo still needs a checkpoint importer
and numerical lowering, while TRM still needs its full
bidirectional-attention lowering. Those absences remain named operator
...
[r-universe](https://rgenomicsetl.r-universe.dev). Installing `Rllm`
